...
from os import system, sys, listdir
...
class Command(BaseCommand):
    ...
    dise_files= {
                "Basic": {"suffix": "basic", "found": 0, "file_name": '', "command": "import_basic"},
                "Facility": {"suffix": "facility", "found": 0, "file_name": '', "command": "import_facilities"},
                "General": {"suffix": "general", "found": 0, "file_name": '', "command": "import_general"},
                "RTE": {"suffix": "rte", "found": 0, "file_name": '', "command": "import_rte"},
                "Teachers": {"suffix": "teachercount", "found": 0, "file_name": '', "command": "import_teachercount"},
                "TotalEnrolment": {"suffix": "totalenrolment", "found": 0, "file_name": '', "command": "import_totalenrolment"}
               }
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('directory', help="Absolute path of directory where DISE files are stored")
        ...
        for filename in listdir(directory):
            for dise_file_type in self.dise_files:
                if dise_file_type in filename:
                    ...
                    file_name = self.convert_xlsx_to_csv(directory+"/"+filename, state, self.dise_files[dise_file_type]["suffix"], academic_year)
    ...
    def call_management_commands(self, academic_year):
        ...
        for dise_file_type in sorted(self.dise_files):
            ...
            if self.dise_files[dise_file_type]["found"]:
                print("Running for: "+dise_file_type)
                management.call_command(self.dise_files[dise_file_type]["command"],self.dise_files[dise_file_type]["file_name"],year=academic_year)
 


    def run_aggregates(self):
        files = ['misc/cluster_aggregation.sql', 'misc/block_aggregation.sql', 'misc/district_aggregation.sql', 'misc/assembly_aggregation.sql', 'misc/parliament_aggregation.sql', 'misc/pincode_aggregation.sql']
