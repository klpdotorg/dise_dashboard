...
class Command(BaseCommand):
    ...
    INDEXES = {
        'School_Code': 0,
        'acyear': 1,
        'Class1_ST_Enr_Boys': 2,
        'Class2_ST_Enr_Boys': 3,
        'Class3_ST_Enr_Boys': 4,
        'Class4_ST_Enr_Boys': 5,
        'Class5_ST_Enr_Boys': 6,
        'Class6_ST_Enr_Boys': 7,
        'Class7_ST_Enr_Boys': 8,
        'Class8_ST_Enr_Boys': 9,
        'Class1_ST_Enr_Girls': 10,
        'Class2_ST_Enr_Girls': 11,
        'Class3_ST_Enr_Girls': 12,
        'Class4_ST_Enr_Girls': 13,
        'Class5_ST_Enr_Girls': 14,
        'Class6_ST_Enr_Girls': 15,
        'Class7_ST_Enr_Girls': 16,
        'Class8_ST_Enr_Girls': 17,
    }
    ...
    def process_row(self, row, year):
        ...
        with transaction.commit_on_success():
            for klass in range(1, 9):
                try:
                    if (int(row[self.INDEXES['Class%s_ST_Enr_Boys' % klass]]) > 0 or
                            int(row[self.INDEXES['Class%s_ST_Enr_Girls' % klass]]) > 0):
                        Enrolment.objects.filter(
                            yearly_data=yearly_data,
                            klass=klass
                        ).update(
                            st_boys=int(row[self.INDEXES['Class%s_ST_Enr_Boys' % klass]]),
                            st_girls=int(row[self.INDEXES['Class%s_ST_Enr_Girls' % klass]]),
                        )
                except Exception as e:
                    print str(e)
