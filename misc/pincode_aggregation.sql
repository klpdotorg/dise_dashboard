...
BEGIN
    ...
    LOOP
        ...
        table_name := 'dise_' || year || '_pincode_aggregations';
        ...
        EXECUTE 'UPDATE ' || table_name || '
            SET centroid=pincode_centroid.centroid
            FROM (SELECT t1.pincode, ST_Centroid(ST_Collect(t2.centroid)) as centroid
                    FROM ' || table_name || ' AS t1,
                    ' || basic_table_name || ' AS t2
                WHERE t2.pincode=t1.pincode
                GROUP BY t1.pincode
                ORDER BY t1.pincode) as pincode_centroid
            WHERE ' || table_name || '.pincode=pincode_centroid.pincode';
    ...
    LOOP
        ...
        EXECUTE 'UPDATE ' || table_name || '
            ...
            FROM
                (
                    SELECT pincode, array_to_json(array_agg(row_to_json(moe_json))) as json_v
                    FROM
                    (
                        SELECT
                            ...
                            count(*) as sch_count
                        FROM ' || basic_table_name || '
                        GROUP BY moe_id, pincode
                        ORDER BY sch_count
                    ) moe_json
                    GROUP BY pincode
                ) moe_json_2
            WHERE moe_json_2.pincode=' || table_name || '.pincode';
    END LOOP;
    RETURN;
END
$BODY$
