...
BEGIN
    ...
    LOOP
        ...
        EXECUTE 'UPDATE ' || table_name || '
            ...
            FROM
                (
                    SELECT parliament_name, array_to_json(array_agg(row_to_json(moe_json))) as json_v
                    FROM
                    (
                        SELECT
                            ...
                            count(*) as sch_count
                        FROM ' || basic_table_name || '
                        GROUP BY moe_id, parliament_name
                        ORDER BY sch_count
                    ) moe_json
                    GROUP BY parliament_name
                ) moe_json_2
            WHERE moe_json_2.parliament_name=' || table_name || '.parliament_name';
    END LOOP;
    RETURN;
END
$BODY$
...
SELECT * FROM aggregate_parliament();
