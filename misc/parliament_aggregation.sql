DROP TABLE IF EXISTS dise_1011_parliament_aggregations;

CREATE TABLE dise_1011_parliament_aggregations AS
...
FROM dise_1011_basic_data
GROUP BY parliament_name
ORDER BY parliament_name;


DROP TABLE IF EXISTS dise_1112_parliament_aggregations;

CREATE TABLE dise_1112_parliament_aggregations AS
SELECT parliament_name,
    Count(school_code) AS sum_schools,
    Sum(CASE WHEN rural_urban = 1 THEN 1 ELSE 0 END) AS sum_rural_schools,
