...
BEGIN
    ...
    LOOP
        ...
        EXECUTE 'UPDATE ' || table_name || '
            ...
            FROM
                (
                    SELECT block_name, district_name, array_to_json(array_agg(row_to_json(moe_json))) as json_v
                    FROM
                    (
                        SELECT
                            ...
                            count(*) as sch_count
                        FROM ' || basic_table_name || '
                        GROUP BY moe_id, block_name, district_name
                        ORDER BY sch_count
                    ) moe_json
                    GROUP BY block_name, district_name
                ) moe_json_2
            WHERE moe_json_2.block_name=' || table_name || '.block_name AND
