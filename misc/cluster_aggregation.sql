CREATE OR REPLACE FUNCTION aggregate_cluster() RETURNS void AS
$BODY$
DECLARE
    years integer[] := array[1011, 1112, 1213, 1314, 1415];
    year integer;
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE 'UPDATE ' || table_name || '
            SET centroid=cluster_centroid.centroid
            FROM (SELECT t1.cluster_name, t1.block_name, t1.district, ST_Centroid(ST_Collect(t2.centroid)) as centroid
                    FROM ' || table_name || ' AS t1,
                    ' || basic_table_name || ' AS t2
                WHERE t2.cluster_name=t1.cluster_name AND
                    t2.block_name=t1.block_name AND
                    t2.district=t1.district
                GROUP BY t1.cluster_name, t1.block_name, t1.district
                ORDER BY t1.cluster_name, t1.block_name, t1.district) as cluster_centroid
            WHERE ' || table_name || '.cluster_name=cluster_centroid.cluster_name AND
                ' || table_name || '.block_name=cluster_centroid.block_name AND
                ' || table_name || '.district=cluster_centroid.district';
    ...
    LOOP
        ...
        EXECUTE 'UPDATE ' || table_name || '
            ...
            FROM
                (
                    SELECT cluster_name, block_name, district_name, array_to_json(array_agg(row_to_json(moe_json))) as json_v
                    FROM
                    (
                        SELECT
                            ...
                            count(*) as sch_count
                        FROM ' || basic_table_name || '
                        GROUP BY moe_id, cluster_name, block_name, district_name
                        ORDER BY sch_count
                    ) moe_json
                    GROUP BY cluster_name, block_name, district_name
                ) moe_json_2
            WHERE moe_json_2.cluster_name=' || table_name || '.cluster_name AND
                moe_json_2.block_name=' || table_name || '.block_name AND
